function applyEffect(selector, options = {}) {
  ...
  //Container *********************************************
    ...
    els.forEach((element) => {
      ...
      const childrenBorder = _options.isContainer
        ? preProcessElements(
            document.querySelectorAll(_options.children.borderSelector)
          )
        : [];
      ...
      element.el.addEventListener("mousemove", (e) => {
        for (let i = 0; i < childrenBorder.length; i++) {
          const x =
            e.pageX - getOffset(childrenBorder[i]).left - window.scrollX;
      ...
      //Children *********************************************
      const children = preProcessElements(
        element.el.querySelectorAll(_options.children.elementSelector)
      );
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        enableBackgroundEffects(
          children[i],
          _options.children.lightColor,
          _options.children.gradientSize
        );
